...
wud_api:
  ...
  user: your_user_basic_authentication
  password: your_password_basic_authentication
  trigger_filter: all
release_notes:
...
| `prefixes`               | string or array | `['local']` | Manage different docker prefix |
...
| `wud_api.user`                | string          | -                                      | User for WUD Basic authentication                     |
...
2. Configure WUD to push updates to MQTT (Mosquitto) and configure WUD in your Home Assistant instance (optional)
...
NOTE: Enable CORS (WUD_SERVER_CORS_ENABLED) for WUD API depending on your setup.
...
2. Check if authentication token is needed and configured or Basic authentication
